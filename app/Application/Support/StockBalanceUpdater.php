...
class StockBalanceUpdater
{
    ...
    public function recomputeForProducts(array $productIds): void
    {
        ...
        $nonSerializedByProduct = StockMovement::query()
            ...
            ->whereIn('product_id', $ids)
            ->selectRaw('product_id')
            ->selectRaw("COALESCE(SUM(CASE WHEN to_status IN ('IN_STOCK', 'RECEIVED') THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN from_status IN ('IN_STOCK', 'RECEIVED') THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) as qty_in_stock_non_serialized")
            ->selectRaw("COALESCE(SUM(CASE WHEN to_status = 'DELIVERED' THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN from_status = 'DELIVERED' THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) as qty_delivered_non_serialized")
            ->selectRaw("COALESCE(SUM(CASE WHEN to_status = 'INTERNAL_USE' THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN from_status = 'INTERNAL_USE' THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) as qty_internal_use_non_serialized")
            ->selectRaw("COALESCE(SUM(CASE WHEN to_status = 'RETURNED' THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN from_status = 'RETURNED' THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) as qty_returned_non_serialized")
            ->selectRaw("COALESCE(SUM(CASE WHEN to_status = 'RETURNED_TO_SUPPLIER' THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN from_status = 'RETURNED_TO_SUPPLIER' THEN CASE WHEN qty_in > qty_out THEN qty_in ELSE qty_out END ELSE 0 END), 0) as qty_returned_to_supplier_non_serialized")
            ->groupBy('product_id')
            ->get()
            ->keyBy('product_id');
